...
class NodeEditorView(QGraphicsView):
    def __init__(self, registry, parent_widget=None):
        ...
        self.active_visualizer = None  # (node, visualizer_index) or None
    ...
    def activate_visualizer(self, node, viz_index):
        """Activate a visualizer, deactivating any currently active one"""
        if self.active_visualizer:
            old_node, old_index = self.active_visualizer
            # Deactivate old visualizer
            old_node.set_visualizer_active(old_index, False)

        self.active_visualizer = (node, viz_index)
        node.set_visualizer_active(viz_index, True)

    def deactivate_visualizer(self):
        """Deactivate the currently active visualizer"""
        if self.active_visualizer:
            node, index = self.active_visualizer
            node.set_visualizer_active(index, False)
