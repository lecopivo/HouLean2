...
class DictType (α : Type u) {A} [ApexType α A] extends ApexType (HashMap String α) Dict where
  get? (dict : Dict) (key : String) : A×Bool
  set (dict : Dict) (key : String) (value : A) : Dict

instance {α A} [ApexType α A] [DictType α] : ApexTypeFlatten (HashMap String α) [.dict] where
...
instance {α A} [ApexType α A] [DictType α] : DictType (HashMap String α) where
  toApex := fun _ => default
  fromApex := fun _ => default
...
run_meta compilerExt.add (.implementedByName ``HashMap.contains ``HashMap.contains.apex_impl
  #[some 1, none, none, none, some 4, some 5]) .global
