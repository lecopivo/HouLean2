...
def withVisualizer (go : VisualizeM Geometry) (visualizerName : String) (visualizerId : Int) :
    struct { output : Geometry, visInfo : HashMap String Int } :=
  let (geo, visInfo, vis) := go (visualizerName, visualizerId) ({}, default)
  let vis := vis.pack |>.setPointAttrib 0 "__visualizer" (1:Int)
  { output := geo.merge vis,
    visInfo := visInfo }
