...
import HouLean.OpenCL.Compiler.Types2
...
partial def compileExpr (e : Expr) : MetaM (TSyntax `clExpr) := do
...
where
  applyImpl (impl : ImplementedBy) (args : Array Expr) : MetaM (TSyntax `clExpr) := do
...
def compileDecl (declName : Name) : MetaM Unit := do
